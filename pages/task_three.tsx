...
const Task3 = () => {
	...
	useEffect(() => {
		...
		let answers = {
			1: localStorage.getItem("taskThree.1"),
			2: localStorage.getItem("taskThree.2"),
			3: localStorage.getItem("taskThree.3")
		}
	...
	const onSubmit = async (data:any) => {
		...
		if (data) {
			...
			if (response.status === 200) {
				localStorage.setItem("taskThree.1", data.answer_1)
				localStorage.setItem("taskThree.2", data.answer_2)
				localStorage.setItem("taskThree.3", data.answer_3)
				setHasSubmitted(true)
				setComplete(3)
			}
		}
	...
	return (<>
		<HeadTemplate title={taskText} description="Truth or Debug Round 4 Task 3"/>
		<div className={styles.container}>
			<div className={styles.content}>
				{task() ? <h1>{taskText} is locked</h1> : <>
					...
					<button onClick={() => {
						router.push('/the_end')
					}} disabled={!hasSubmitted}>{hasSubmitted && '> '}enter the_end</button>
				</>}
			</div>
			<Footer />
		</div>
